...
import { DEFAULT_NOTIFICATION_PREFERENCES, loadNotificationPreferences, saveNotificationPreferences, preferenceAllows, type NotificationPreferences } from "../lib/notificationPreferences";
import { enableWebPush, syncWebPushSubscription } from "../lib/pushNotifications";
...
export function isUniversityEmail(email: string) {
  ...
  return UNIVERSITY_EMAIL_DOMAIN.some((domain) =>
...
const defaultCtx: AppContextType = {
  ...
  notificationPreferences: DEFAULT_NOTIFICATION_PREFERENCES, updateNotificationPreferences: async () => {}, enableBrowserNotifications: async () => {},
};
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const loadUserData = useCallback(async (userId: string, authUser?: { email?: string; user_metadata?: Record<string, unknown> }) => {
    ...
    try { setNotificationPreferences(await loadNotificationPreferences(userId)); } catch (error) { console.warn("Could not load notification preferences:", error); }
  ...
  const updateNotificationPreferences = useCallback(async (next: NotificationPreferences) => {
    ...
    try { await saveNotificationPreferences(user.id, next); } catch (error) {
  ...
  const enableBrowserNotifications = useCallback(async () => {
    ...
    const next = { ...notificationPreferences, browserNotifications: true };
    setNotificationPreferences(next);
 ...
 useEffect(() => {
  ...
  const setupNotifications = async () => {
    try {
      ...
      console.log("Notification permission not enabled:", error);
    }
  };
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        notificationPreferences, updateNotificationPreferences, enableBrowserNotifications,
      }}
    >
      {children}
    </AppContext.Provider>
