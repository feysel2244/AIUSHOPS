...
export default function ServiceDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadService() {
      ...
      const { data } = await supabase
        .from("services")
        .select("*,shops!inner(*,profiles(name,department,year,whatsapp))").is("shops.deleted_at", null).is("deleted_at", null)
        .eq("slug", slug)
        .maybeSingle();
  ...
  const serviceSeo = (
  <Helmet>
    <title>{sv.name} | AIU Market</title>

    <meta
      name="description"
      content={`Book ${sv.name} from ${sv.shopName} on AIU Market. ${sv.description}`}
    />

    <meta
      property="og:title"
      content={`${sv.name} | AIU Market`}
    />
    ...
    <meta
      property="og:url"
      content={`https://aiumarket.com/service/${sv.slug}`}
    />
