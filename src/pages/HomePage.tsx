...
`).eq("status", "approved").is("deleted_at", null).order("rating", { ascending: false }).limit(6),
        ...
        supabase.from("products").select("*,shops!inner(*,profiles(name,department,year,whatsapp))").is("shops.deleted_at", null).is("deleted_at", null).eq("is_promoted", true).gt("promoted_until", now).limit(4),
        supabase.from("services").select("*,shops!inner(*,profiles(name,department,year,whatsapp))").is("shops.deleted_at", null).is("deleted_at", null).eq("is_promoted", true).gt("promoted_until", now).limit(4),
      ]);
          ...
          {loading ? (
            ...
            <HorizontalScroll>
              {favouriteShopsList.map((shop) => (
                <div key={shop.id} className="flex-shrink-0 w-40 md:w-40">
                  <ShopCard shop={shop} />
                </div>
              ))}
            </HorizontalScroll>
