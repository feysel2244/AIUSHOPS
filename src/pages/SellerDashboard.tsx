  ...
  async function handleAddProduct() {
    ...
    if (productImageFiles.length > 0) {
      ...
      for (let i = 0; i < productImageFiles.length; i++) {
        ...
        try {
          const url = await uploadImage("product-images", `${shop.id}/${newProduct.id}-${i}.${ext}`, file);
          urls.push(url);
        } catch (uploadErr) {
