...
function toReview(row: any): Review {
  ...
  return {
    id: row.id,
    author: profile?.name || "AIU Student",
    avatar: profile?.avatar_url || avatarFallback,
    rating: Number(row.rating) || 0,
    date: row.created_at
      ? new Date(row.created_at).toLocaleDateString("en-MY")
      : "",
    text: row.text || "",
    product: product?.name || service?.name || "Shop order",
  };
}
...
export default function ShopPage() {
    ...
    async function loadShop() {
      ...
      const [
        ...
        supabase
          .from("products")
          .select(
            "*,shops!inner(*,profiles(name,department,year,whatsapp))"
          )
        ...
        supabase
          .from("reviews")
          .select(
            "id,rating,text,created_at,profiles(name,avatar_url),products(name),services(name)"
          )
          .eq("shop_id", shopRow.id)
          .order("created_at", { ascending: false }),
      ]);
      ...
      /*
       * IMPORTANT:
       * Do not silently treat a Supabase review query error as
       * "0 reviews". We still set the reviews that were returned,
       * but the console will now show the real database error.
       */
      const convertedReviews = (reviewRows ?? [])
        .map(toReview)
        .filter(
          (review) =>
      ...
      setProducts(
        (productRows ?? []).map((row) => toProduct(row as ProductRow))
      );

      setServices(
        (serviceRows ?? []).map((row) => toService(row as ServiceRow))
      );
  ...
  if (!shop) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link
          to="/browse"
          className="px-6 py-2 bg-[#1C3270] text-white rounded-lg text-sm font-medium hover:bg-[#0F1F4A]"
        >
          Browse All Shops
        </Link>
      </div>
  ...
  function sortItems<T extends { price: number; rating: number }>(
    items: T[]
  ) {
    if (sort === "price_asc") {
   ...
   * ============================================================
   *
   * Use the actual review rows returned from the database.
   *
   * Example:
   * 5-star = 4 reviews
   * 4-star = 3 reviews
   ...
   * Total = 10
   *
   * 5-star percentage = 40%
   */

  const ratingSource = reviews
  ...
  const totalRatings = ratingSource.length;

  const ratingDist = [5, 4, 3, 2, 1].map((star) => {
    const count = ratingSource.filter(
  ...
  function handleMessage() {
    ...
    if (shop.seller.whatsapp) {
      window.open(
        `https://wa.me/${shop.seller.whatsapp}`,
        "_blank"
      );
    }
  }
  ...
  return (
    <>
      <Helmet>
        <title>{shop.name} | AIU Market</title>

        <meta
          name="description"
          content={`Visit ${shop.name} on AIU Market. Browse products and services from this seller. ${shop.description}`}
        />

        <meta
          property="og:title"
          content={`${shop.name} | AIU Market`}
        />
        ...
        <meta
          property="og:url"
          content={`https://aiumarket.com/shop/${shop.slug}`}
        />
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="relative -mt-10 mb-6">
              ...
              <div className="flex items-center gap-2 pt-12">
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium border transition-colors whitespace-nowrap ${
                    followed
                      ? "border-[#1C3270] text-[#1C3270] bg-[#1C3270]/5"
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-sm font-semibold bg-[#25D366] text-white hover:bg-[#1ebe5d] transition-colors flex items-center gap-1.5 whitespace-nowrap"
                >
            ...
            <div className="mt-3">
              <div className="flex items-center gap-2 flex-wrap">
                <h1
                  className="text-xl md:text-2xl font-bold text-stone-900 leading-tight"
                  style={{ fontFamily: "Lora, serif" }}
                >
                  {shop.name}
                </h1>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
              ...
              {["All", "Products", "Services"].map((t) => (
                <button
                  ...
                  className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
                    tab === t
                      ? "bg-white text-[#1C3270] shadow-sm"
            ...
            <select
              ...
              className="w-full sm:w-auto h-9 px-3 border border-stone-200 rounded-lg text-sm text-stone-600 focus:outline-none focus:border-[#1C3270] bg-white"
            >
          ...
          visibleServices.length === 0 ? (
            ...
            <div className="space-y-8 mb-12">
              {visibleProducts.length > 0 && (
                <div>
                  {tab === "All" && (
                    <h3 className="font-bold text-stone-700 text-sm uppercase tracking-wide mb-3">
                      Products
                    </h3>
                  )}
          ...
          <div className="bg-white rounded-xl border border-stone-100 p-5 mb-8">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#1C3270] text-white flex items-center justify-center font-bold">
                {shop.seller.name.charAt(0)}
              </div>
